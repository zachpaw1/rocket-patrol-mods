class Play extends Phaser.Scene {
    ...
    create() {
        ...
        // borders
        this.add.rectangle(0, 0, game.config.width, borderUISize, 0x424242).setOrigin(0, 0);
        this.add.rectangle(0, game.config.height - borderUISize, game.config.width, borderUISize, 0x424242).setOrigin(0, 0);
     //   this.add.rectangle(0, 0, borderUISize, game.config.height, 0x424242).setOrigin(0, 0);
     //  this.add.rectangle(game.config.width - borderUISize, 0, borderUISize, game.config.height, 0x424242).setOrigin(0, 0);
        ...
        // add jet
        this.jet = new Jet(this, game.config.width, borderUISize * 9 + borderPadding * 2, 'jet', 0, 50).setOrigin(0, 0);
    ...
    update() {
        ...
        if (this.checkCollision(this.p2Rocket, this.jet)) {
            this.p2Rocket.reset();
            this.shipExplode(this.jet);
        }
    }
    ...
    checkCollision(rocket, jet) {
        // simple Axis-Aligned Bounding Boxes (AABB) checking
        if (rocket.x < jet.x + jet.width && rocket.x +rocket.width > jet.x && rocket.y < jet.y + jet.height && rocket.height + rocket.y > jet.y) {
            return true;
        } else {
            return false;
        }
    ...
    shipExplode(jet) {
        ...
        boom.anims.play('explode');                 // play explode animation
        boom.on('animationcomplete', () => {        // callback after animation completes
            jet.reset();                            // reset ship position
            jet.alpha = 1;                          // make ship visible again
            boom.destroy();                         // remove explostion sprite
        });
